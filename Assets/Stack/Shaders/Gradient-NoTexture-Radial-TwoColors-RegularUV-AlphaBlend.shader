Shader "Gradient/No Texture/Radial/Two Colors/Regular UV/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(_Color,_Color2,sqrt( (i.uv.x*i.uv.x)+(i.uv.y*i.uv.y) )+_Offset ));	//Output radial gradient
			}
