Shader "Gradient/No Texture/Diamond/Two Colors/Regular UV/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(_Color,_Color2,abs(i.uv.x)+abs(i.uv.y)+_Offset ));	//Output diamond gradient
			}
