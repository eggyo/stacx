Shader "Gradient/No Texture/Horizontal/Two Colors/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			VertexToFragment vert(AppData v) {
				...
				//o.uv = v.texcoord.xy;
				v.texcoord.xy = half2((v.texcoord.x+_UVXOffset)*_UVScale,(v.texcoord.y+_UVYOffset)*_UVScale);
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(_Color,_Color2,abs(i.uv.x)));	//Output horizontal gradient
			}
